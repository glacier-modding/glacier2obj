...
pub fn main() {
    ...
    if args.len() < 5 {
        eprintln!("Usage: cargo run <path to a Retail directory> <game version (H2016 | HM2 | HM3)> <path to navkit.json file> <path to a Runtime directory> <path to output aloc directory>");
        return;
    }
    ...
    println!("Done building output.nav.json and extracting alocs and pfBoxes from scenario.");
    io::stdout().flush().unwrap();
}
